...
| Anything triggered by "após sofrer danos …" | **Real** — a `*RacialAbility` constant overriding `SkillCompetencyAbility#resolveDamageTakenBlessings(holder, finalDamage)`, listed by `getRacialAbilities()`. The ability returns whole `Blessing`s and `DamageService#notifyDamageTaken` applies them, so any `ModifierType` works, not just healing. `TrollsRacialAbility#REGENERACAO_REATIVA` is the reference (a `ModifierType.REGENERATION` Blessing: 2PV, Vigor Rodadas, `totalLimit` = the triggering damage); see the `damage-and-combat` skill for how far "de fontes inimigas" is honoured. |
