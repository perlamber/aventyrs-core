...
| **Round-scoped Attribute bonuses — *partly built*** | `AttributeValue` has only `base`/`racialBonus`/`variable`, all permanent. But `ModifierType` now has one `<ATTR>_BONUS` constant per `AttributeDomain` (`AttributeDomain#getBonusModifierType()`), and a `TemporaryBonus` of one — or a permanent `Feat#resolveAttributeBonus(AttributeDomain, Character)` grant (`VampiricoFeat#MESTRE_VAMPIRO`, the first Talento to grant an Atributo bonus, `DOM_DE_MIRCALLA` the first round-scoped one) — is read in **exactly one place**: `AbstractSkillInteraction`, after `getValueForRoll`, so the bonus reaches a Perícia roll governed by that Atributo and nothing else. HP/PM/PD/Defesa/Conjuração still read `AttributeValue#getTotal()` directly — promote to a `Character#getEffectiveAttributeTotal(domain)` those sites call when a second consumer needs it. |
...
- **Catalog vs owned copy** — the split `AventyrTitle`'s javadoc documents. The **catalog** side
  ...
  service is a delegate. **Two paths, and the difference is the whole point**: `ItemForgery.by(
  ...
  `Item#isDonatedByAventyr()`. That marker is deliberately not a `null producedByCharacterId`,
  which only means "maker unknown" (loot, a bridge copy); a donation says there was no making.
  It is deliberately *not* a method on `EquipmentCraftingService`: every number that service
  exists to compute is what a donation skips.
- **A forge is assembled a decision at a time, and each one is priced.** `ItemForgery#setMasterpiece`
  ...
  (worth is not cost). **Every catalog Preço modifier is 0 today**, so a real forge's total is its
  base Preço; the arithmetic is real, and authoring one modifier prices every forge using it. The
  cost is **reported, not spent** — no PE economy. The mutators validate nothing; `validate()`
  ...
  **Acquiring a Talento and using it are different questions.** `FeatRequirements`/`isEligible`
  ...
  change to the forge.
  Still unmodeled: a PE economy, the offensive Obra-Prima/Aprimoramento catalogs, any Pedra
