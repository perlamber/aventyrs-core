...
| **Resistência a Críticos (RC) — *round-scoped grant only*** | `ModifierType.CRITICAL_RESISTANCE` exists and `AbstractSkillInteraction` subtracts the *attack target's* `getTemporaryBonus(CRITICAL_RESISTANCE)` from the attacker's summed Margem Crítica Menor widening (so it also covers `AttackDelivery`, not the `AttackReceiver` mirror). One instance = a value of 2, per `docs/rules/defesas-e-resistencias.txt`. `AnaoFeat#VIGOR_DO_INVERNO` grants one at combat start. **Still missing:** the "-1 à Margem Crítica Maior" clause has no expression (this ruleset models no Acerto Crítico Maior margin at all); "não-PRIMORDIAL" scoping isn't on the crit path; RC only cancels attacker widening, never pushing a crit below baseline (the "até o mínimo de 17" floor is approximated); and a *permanent* RC source (`MonstruosoFeat`, `ElementalFeat`, `GorgonaFeat`, `Troll`'s Anatomia Vegetal, `ProfissaoCompetencyAbility`/`FORJA_VULCANA`) needs its own scan added on the attacker's crit path — each is form-gated or otherwise blocked besides. Not the same as `Race#getCriticalEffectImmunities()` (an all-or-nothing `CriticalEffectType` filter). |
...
| **"Spend a resource to enter a timed state" — *built*** | `ActiveAbility` + `ActiveAbilityService#activate` do the whole cycle: validate the ability is held (by reference), check PA/PM/**PV** affordability (`getHitPointCost()`, refused if self-fatal — "Ação Livre" = `getActionPointCost()` 0), spend, and apply every `TemporaryEffect` from `resolveEffects(Character)`. A **Talento** contributes one via `Feat#resolveActiveAbility()` (must return a stable singleton — `Character#getActiveAbilities()` aggregates `getFeats()` live and `activate` matches by `==`); an `AttributeAbility` still copies its own onto `Character.activeAbilities` at acquisition. `VampiricoFeat`'s four Poderes Vampíricos (`OSTEOMANCIA`/`CELERIDADE_VAMPIRICA`/`ARMAMENTO_DE_ORLOK`/`DOM_DE_MIRCALLA`, via `PoderVampiricoActiveAbility`) and `FocusAbility#CONCENTRACAO_PROFUNDA` are the consumers. Still not covered: a **form** state (a persistent alternate shape, not a timed bonus — `DRACONATO`, `METAMORFOSE_DRACULEA`, HomemFera), and a per-Rodada effect that needs the Scene's neighbours (`TemporaryEffect#applyRoundEffect(CombatantSheet)` gets the holder's sheet alone — `PRESENCA_DE_CARMILLA`). The flight TODOs in `Aviano`/`DraconicoFeat`/`FeericoFeat` can now cite the flight *sub-stat* as the blocker, not the activation. |
