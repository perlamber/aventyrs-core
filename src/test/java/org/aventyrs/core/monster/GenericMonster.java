 ...
 *
 * <p>The numbers rise together across the tiers (Attributes, Graduações, Defesas, the general GD
 * and the Atenção GD — how sharp an eye each has for a hidden character),
...
public enum GenericMonster implements MonsterTemplate {
    ...
    CAPANGA("Capanga",
            ...
            SizeCategory.ZERO, 13, 11, SkillDifficulty.of(DifficultyLevel.EASY, 0),
            SkillDifficulty.of(DifficultyLevel.VERY_EASY, 0), 4),
    ...
    ATIRADOR("Atirador",
            ...
            SizeCategory.ZERO, 12, 11, SkillDifficulty.of(DifficultyLevel.EASY, 1),
            SkillDifficulty.of(DifficultyLevel.VERY_EASY, 1), 3),
    ...
    BRUTAMONTES("Brutamontes",
            ...
            SizeCategory.PLUS_ONE, 17, 12, SkillDifficulty.of(DifficultyLevel.MEDIUM, 2),
            SkillDifficulty.of(DifficultyLevel.EASY, 0), 6),
    ...
    CONJURADOR("Conjurador",
            ...
            SizeCategory.ZERO, 13, 19, SkillDifficulty.of(DifficultyLevel.MEDIUM, 3),
            SkillDifficulty.of(DifficultyLevel.EASY, 2), 4),
    ...
    ABERRACAO("Aberração",
            ...
            SizeCategory.PLUS_THREE, 24, 22, SkillDifficulty.of(DifficultyLevel.VERY_HARD, 4),
            SkillDifficulty.of(DifficultyLevel.HARD, 1), 8);
    ...
    /**
     * The GD it presents on every Perícia but Atenção — what its attacks present to a defender's
     * Esquiva e Aparar roll, and what it hides at. See {@link MonsterTemplate#getGeneralDifficulty()}.
     */
    private final SkillDifficulty generalDifficulty;

    /**
     * The Atenção GD each tier presents to a hidden character — the one Perícia every archetype
     * names a GD for, on the same single dial as the rest, rising with the tier rather than with
     * the Atenção Graduação the constants happen to carry. See {@link MonsterTemplate#getPerception()}.
     */
    ...
    public Map<SkillType, SkillDifficulty> getSkillDifficulties() {
        return Map.of(SkillType.ATTENTION, attentionDifficulty);
    }
