...
/**
 * Requests a Dirigir e Cavalgar Perícia test. Only meaningful for a {@link CharacterSheet}
 * target, since only a Character carries attributes and skills — this Interaction computes
 * the roll bonus itself, looking up the target's own trained Dirigir e Cavalgar
 * CharacterSkill (or defaulting to untrained, which carries {@link Skill#UNTRAINED_PENALTY}).
 * Which of Dirigir e Cavalgar's specializations the roll is for doesn't change the bonus —
 * see {@link DirigirECavalgarSpecialization} — so it isn't tracked here.
 */
public class DirigirECavalgarInteraction implements Interaction<CharacterSheet> {
    ...
    public InteractionResult applyTo(final CharacterSheet target) {
        ...
        CharacterSkill dirigirECavalgarSkill = findCharacterSkill(character);
        int graduationValue = dirigirECavalgarSkill.getGraduation().getGraduationValue();

        int bonus = characterSkillService.getValueForRoll(dirigirECavalgarSkill, character.getAttributes(), character.getRace());
        ...
        List<SkillExcellency> unlockedExcellencies = SkillExcellency.unlockedBy(DirigirECavalgarExcellency.class, graduationValue);
        ...
        int difficultyReduction = SkillExcellency.totalDifficultyReduction(DirigirECavalgarExcellency.class, graduationValue);
    ...
    /**
     * The Character's own CharacterSkill for Dirigir e Cavalgar, or a fresh one carrying
    ...
    private CharacterSkill findCharacterSkill(final Character character) {
        CharacterSkill trained = character.getSkills().get(SkillType.DIRIGIR_E_CAVALGAR);
        if (trained != null) {
            return trained;
        }
        return CharacterSkill.builder()
                .skill(new DirigirECavalgar())
