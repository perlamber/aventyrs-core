...
public enum MedicinaECuraCompetencyAbility implements SkillCompetencyAbility {
    ...
    // Especialização *and* 4 Graduações to acquire — both unenforced, same as
    // AtletismoCompetencyAbility.ATLETA_VERSATIL, since no eligibility-validation service
    // exists for SkillCompetencyAbility (and Especialização is a plain String on
    // CharacterSkill, not independently validatable either).
