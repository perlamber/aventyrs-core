...
/**
 * Requests a Profissão Perícia test. Only meaningful for a {@link CharacterSheet} target,
 * since only a Character carries attributes and skills — this Interaction computes the roll
 * bonus itself, looking up the target's own trained Profissão CharacterSkill (or defaulting
 * to untrained, which carries {@link Skill#UNTRAINED_PENALTY}). Which of Profissão's
 * specializations the roll is for doesn't change the bonus — see
 * {@link ProfissaoSpecialization} — so it isn't tracked here.
 */
public class ProfissaoInteraction implements Interaction<CharacterSheet> {
    ...
    public InteractionResult applyTo(final CharacterSheet target) {
        ...
        CharacterSkill profissaoSkill = findCharacterSkill(character);
        int graduationValue = profissaoSkill.getGraduation().getGraduationValue();

        int bonus = characterSkillService.getValueForRoll(profissaoSkill, character.getAttributes(), character.getRace());
    ...
    /**
     * The Character's own CharacterSkill for Profissão, or a fresh one carrying
    ...
    private CharacterSkill findCharacterSkill(final Character character) {
        CharacterSkill trained = character.getSkills().get(SkillType.PROFISSAO);
        if (trained != null) {
            return trained;
        }
