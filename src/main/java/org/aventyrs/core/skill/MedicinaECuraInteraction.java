...
/**
 * Requests a Medicina e Cura Perícia test. Only meaningful for a {@link CharacterSheet}
 * target, since only a Character carries attributes and skills — this Interaction computes
 * the roll bonus itself, looking up the target's own trained Medicina e Cura CharacterSkill
 * (or defaulting to untrained, which carries {@link Skill#UNTRAINED_PENALTY}). Which of
 * Medicina e Cura's specializations the roll is for doesn't change the bonus — see
 * {@link MedicinaECuraSpecialization} — so it isn't tracked here.
 */
public class MedicinaECuraInteraction implements Interaction<CharacterSheet> {
    ...
    public InteractionResult applyTo(final CharacterSheet target) {
        ...
        CharacterSkill medicinaECuraSkill = findCharacterSkill(character);
        int graduationValue = medicinaECuraSkill.getGraduation().getGraduationValue();

        int bonus = characterSkillService.getValueForRoll(medicinaECuraSkill, character.getAttributes(), character.getRace());
        ...
        List<SkillExcellency> unlockedExcellencies = SkillExcellency.unlockedBy(MedicinaECuraExcellency.class, graduationValue);
        bonus += modifierResolver.sumModifiers(unlockedExcellencies, ModifierType.SKILL_ROLL_BONUS);

        int difficultyReduction = SkillExcellency.totalDifficultyReduction(MedicinaECuraExcellency.class, graduationValue);
    ...
    /**
     * The Character's own CharacterSkill for Medicina e Cura, or a fresh one carrying
    ...
    private CharacterSkill findCharacterSkill(final Character character) {
        CharacterSkill trained = character.getSkills().get(SkillType.MEDICINA_E_CURA);
        if (trained != null) {
            return trained;
        }
        return CharacterSkill.builder()
                .skill(new MedicinaECura())
