...
public enum CurandeiroAbility implements AventyrTitleAbility {
    ...
    // Requer 1 Especialização de 'Curandeiro'. 2PD, Ação Livre.
    // TODO: "-2PA" on the next healing activation or Magia Natural/Divina — no Título-side
    //  activation-time reduction reaches SpellCastingServiceImpl#resolveActivationTime, and nothing
    //  scopes a reduction to "a próxima" one.
    CURANDEIRO_VELOZ(
            ...
            false, fixed(2), EgoCost.NONE, ActionCost.FREE_ACTION, Optional.empty(), 1, 0),
    ...
    // TODO: "mesmo que você esteja inconsciente ou morto" — nothing gates acting on CharacterStatus, so
    //  there is no refusal to lift (the same gap SobrevivenciaFeat#PERMANECER_CONSCIENTE cites).
    // TODO: "custam o dobro de PM e PD" — no Título hook scales a Magia's Mana or a Habilidade's PD cost.
    // TODO: "por uma quantidade de Rodadas igual à metade das suas Graduações em Medicina e Cura" —
    //  counted from what? Nothing records when the holder fell.
    BENCAO_DE_BOROS(
