 ...
 * cascading-overload shape (an {@link Interaction}&lt;{@link CombatantSheet}&gt; computing an
 ...
 * entry) and calls {@code CombatantSheet#grantTemporaryBonus} on each — this class doesn't
 ...
 * consumed that {@code ModifierType} at the time. It does now: {@code
 * character.services.DefenseService} sums every active {@code DEFESAS}-typed {@code
 * TemporaryBonus}, so this Blessing has been real since that service landed — with no change at
 * this granting site, which is exactly what granting it ahead of a reader bought.
 ...
 */
public class GritoDeGuerraVulcanoInteraction implements Interaction<CombatantSheet> {
    ...
    @Override
    public InteractionResult applyTo(final CombatantSheet actor) {
    ...
    public InteractionResult applyTo(final CombatantSheet actor, final SceneContext sceneContext) {
        ...
        return InteractionResult.builder()
                .resultStatus(hitPointsService.getStatus(actor))
                .blessings(List.of(
