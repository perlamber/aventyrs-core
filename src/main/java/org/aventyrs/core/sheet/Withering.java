 ...
 * CombatantSheet#applyEffect(TemporaryEffect)}. Counts down in Rodadas via its shared
 ...
 *
 * <p>Unlike {@link Bleeding} (plain {@link CombatantSheet#applyDamage(int)}), each Rodada
 * drains via {@link CombatantSheet#applyCurseDamage(int)} instead — Definhar's own rules
 ...
 * explicitly "não cumulativo", so {@link CombatantSheet#applyEffect} replaces any existing
